...
// RunMonitorFIM checks critical files for unexpected changes (size, mtime).
func RunMonitorFIM(_ map[string]interface{}) (string, error) {
	cmd := "echo '---PASSWD---'; ls -la /etc/passwd /etc/shadow /etc/sudoers /etc/ssh/sshd_config 2>/dev/null; echo '---CRON---'; ls -laR /etc/cron* 2>/dev/null | head -30"
	return run(cmd)
}
...
func RunMonitorRootkit(_ map[string]interface{}) (string, error) {
	cmds := []string{
		...
		"echo '---MODULES---'; lsmod 2>/dev/null | grep -E 'hide|root|kit'",
		"echo '---NET---'; ss -tupn 2>/dev/null | grep -E '0\\.0\\.0\\.0|:::'",
		"echo '---SUID---'; find / -type f -perm -4000 2>/dev/null | xargs -I{} ls -la {} 2>/dev/null | head -20",
	}
