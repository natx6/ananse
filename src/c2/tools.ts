...
export function createC2TaskCreateTool() {
  return tool({
    ...
    inputSchema: z.object({
      ...
      type: z.string().min(1).describe("Task type: recon_all, credential_all, privesc_all, exploit_all, monitor_all, persistence_all, gather_all (runs ALL intel modules at once), or individual: recon_processes, recon_network, recon_users, recon_cron, recon_suid, privesc_sudo, privesc_writable, privesc_kernel, persistence_ssh, persistence_startup, exploit_packages, exploit_services, monitor_fim, monitor_rootkit, brute_sudo, brute_ssh, brute_local, brute_all, credential_shadow, credential_browsers, credential_ssh_keys, credential_configs, lateral_ssh, lateral_all, collect_keylog, collect_screenshot, collect_clipboard, collect_all, bypass_amsi, bypass_etw, bypass_all"),
