...
// Helpers
// ---------------------------------------------------------------------------
function findAffected(graph, targetPath) {
    ...
    const transitive = [];
    for (const [file, deps] of Object.entries(graph)) {
        if (file === targetPath || reversePaths.has(file))
            continue;
        for (const dep of deps) {
...
function formatBlastRadius(targetPath, graph) {
    const targetEntry = Object.entries(graph).find(([k]) => k === targetPath);
    ...
    const transitive = [];
    for (const [file, deps] of Object.entries(graph)) {
        if (file === targetPath || revSet.has(file))
            continue;
        for (const dep of deps) {
    ...
    const transLines = transitive.length
        ...
        : ["└── (none)"];
    const allFiles = [targetPath, ...reverse, ...transitive, ...forward.map((d) => d.resolvedPath).filter((x) => !!x)];
    const text = [
...
// runRefactor
// ---------------------------------------------------------------------------
export async function runRefactor(targetPath, description, config) {
