...
function extractCompilerErrors(raw) {
    const errors = [];
    // TypeScript
    let m;
...
// ---------------------------------------------------------------------------
function isCompileCommand(command) {
 ...
 */
export function condenseError(raw, command) {
    if (!raw)
        return "";
    // Step 1: strip ANSI
 ...
 */
export function detectAndCondense(output, command) {
    if (!output || output.trim() === "(no output)")
        return null;
    // Check for error indicators
    const errorKeywords = /\b(error|failed|fatal|panic|exception|traceback|killed|exit code)\b/i;
    if (!errorKeywords.test(output))
        return null;
