...
program
    ...
    try {
        ...
        if (du)
            console.log(`  └── Disk      ${picocolors.dim(du.split(/\s+/)[0])}`);
    }
    catch {
        ...
        if (base) {
            try {
                ...
                if (res.ok) {
                    ...
                    if (remaining)
                        console.log(`  └── Rate limit ${picocolors.yellow(remaining)} remaining`);
                    else
                        console.log(`  └── Rate limit ${picocolors.dim("unknown")}`);
                }
                else {
...
program
    ...
    try {
        ...
        const url = execSync(`gh pr create --title "${title.replace(/"/g, '\\"')}" --body ""`, { encoding: "utf-8" }).trim();
